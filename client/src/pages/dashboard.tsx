...
export default function Dashboard() {
  ...
  const [fetchingMore, setFetchingMore] = useState(false); // Indikator untuk batch tambahan
    ...
    const fetchAllBatches = async () => {
      try {
        ...
        do {
          ...
          if (!isMounted) return; // Cek jika komponen sudah unmounted

          // Perbarui state dengan data batch baru
          setCustomers((prev) => [...prev, ...data]);

          // Matikan loading awal setelah batch pertama
          if (page === 1) {
            setLoading(false);
            if (totalPages > 1) setFetchingMore(true); // Aktifkan indikator jika ada batch lain
          }

          page++;
        } while (page <= totalPages);

        // Semua batch selesai, matikan indikator fetching
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      <div className="container mx-auto p-6 space-y-6">
          ...
          <Card className="bg-gradient-to-r from-green-500 to-green-600 text-white">
            ...
            <CardContent>
              ...
              <p className="text-xs text-green-100">
                Age range: {customers.reduce((min, c) => Math.min(min, c.age), Infinity)} -{" "}
                {customers.reduce((max, c) => Math.max(max, c.age), -Infinity)}
              </p>
            </CardContent>
          </Card>
        ...
        {/* Indikator Fetching Batch Tambahan */}
        {fetchingMore && (
